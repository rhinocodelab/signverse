...
export const GeneralAnnouncementISL: React.FC = () => {
    ...
    const fetchAnnouncements = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/general-announcements/?${params}`)
            if (response.ok) {
    ...
    const fetchCategories = useCallback(async () => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/general-announcements/categories/list`)
            if (response.ok) {
                const data = await response.json()
    ...
    const handlePlayVideo = (announcementId: number) => {
        const videoUrl = `${process.env.NEXT_PUBLIC_API_URL}/general-announcements/${announcementId}/video/stream`
